...
def write_csv_table(
    ...
    *,
    ...
    if not normalized:
        return None
    fieldnames = ordered_columns(normalized, preferred_columns)
    ...
    if not rows and not columns:
        ...
        for row_index, row in enumerate(rows, start=2):
            ...
            for column_index, column_name in enumerate(columns, start=1):
                ...
                style_id = "2" if _is_wrap_column(column_name) or len(str(value)) > 48 else None
...
def write_xlsx_workbook(
    ...
    *,
    ...
    for requested_name, rows in sheets.items():
        ...
        columns = ordered_columns(ordered_rows, preferred_columns.get(requested_name, ()))
        if not ordered_rows and not columns:
            continue
        sheet_name = _sanitize_sheet_name(requested_name, used_names)
        normalized.append((sheet_name, ordered_rows, columns))
    if not normalized:
    ...
    for index, (sheet_name, _rows, _columns) in enumerate(normalized, start=1):
        SubElement(
            sheets_element,
            "sheet",
            name=sheet_name,
    ...
    with ZipFile(path, "w", compression=ZIP_DEFLATED) as archive:
        ...
        for index, (sheet_name, rows, columns) in enumerate(normalized, start=1):
            archive.writestr(
                f"xl/worksheets/sheet{index}.xml",
                _worksheet_xml(sheet_name, rows, columns),
            )
    return path
